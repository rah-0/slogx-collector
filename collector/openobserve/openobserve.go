...
func (d *Destination) mapTimestamp(record json.RawMessage) (json.RawMessage, error) {
	...
	if len(trimmed) < 2 || trimmed[0] != '{' || !json.Valid(record) {
	...
	mapped := make(json.RawMessage, 0, len(record)+40)
	mapped = append(mapped, record[:end]...)
	mapped = append(mapped, `,"_timestamp":`...)
...
func (d *Destination) timestamp(source json.RawMessage) (int64, error) {
	...
	layouts := []string{time.RFC3339Nano, "2006-01-02 15:04:05.000000"}
