...
func TestInputErrorsAreMatchableWithoutExposingInput(t *testing.T) {
	for _, test := range []struct {
		...
		{"invalid JSON", strings.NewReader("{}\n{\"secret-token\":"), collector.ErrInvalidJSONRecord},
		{"read failure", io.MultiReader(strings.NewReader("{}\n"), errorTestReader{}), collector.ErrInputRead},
	} {
...
func TestJournalErrorsAreMatchable(t *testing.T) {
	...
	t.Run("destination mismatch", func(t *testing.T) {
		...
		if err := run(t, dir, "second-target"); !errors.Is(err, collector.ErrJournalDestinationMismatch) {
	...
	t.Run("invalid checkpoint", func(t *testing.T) {
		...
		if err := run(t, dir, "target"); !errors.Is(err, collector.ErrJournalInvalidCheckpoint) {
	...
	t.Run("checksum mismatch", func(t *testing.T) {
		...
		if err := run(t, dir, "target"); !errors.Is(err, collector.ErrJournalChecksum) {
