...
var (
	...
	ErrUnsupportedDestination   = errors.New("-destination must select an output adapter: openobserve (OpenObserve integration) or otlp (OTLP HTTP/JSON traces)")
	ErrTraceLogOptions          = errors.New("-timestamp-field and -timestamp-layout apply only to OpenObserve JSON logs")
	ErrTraceEndpointDestination = errors.New("-traces-endpoint requires -destination openobserve")
	ErrTraceResourceDestination = errors.New("-resource requires -destination otlp or -traces-endpoint")
	ErrInvalidResource          = errors.New("-resource must use Name=Value with a nonempty name")
